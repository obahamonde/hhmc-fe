...
<template>
<main v-if="state.user && showChat" class=" scroll-y-auto pt-20 gap-4 min-w-32  h-full pb-24 sh w-90vw mx-24 bg-gray-300 rounded lg br m-4"
>
...
<WebSocket :url="'wss://hhmc.aiofauna.com/api/chat/'+state.user.ref" ref="wsRef" @receive="handleReceive($event)">
</WebSocket>	
</div>
<div class="col start" v-if="state.user">
  <div v-for="msg in state.messages" class="row center">
		<img :src="msg.role == 'user' ? state.user.picture : '/chatbot.svg'" class="x4 rf br m-8 scale" :class="msg.role=='user'?'':''" />
		<p :class="msg.role == 'assistant' ? 'col':'col end'"
...
<p class="flex row items-center justify-center bottom-0 fixed  w-90vw bg-primary py-2">
